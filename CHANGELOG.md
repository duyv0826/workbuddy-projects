...
- **动漫图片测试下载**：样例复现指引（说明如何按需用下载器生成 SFW 样图；样图不随仓库分发）。
...
### 移除（Removed）
- 将 `动漫图片测试下载/` 的 2 张第三方版权样图从工作区与 git 历史彻底清除（`git filter-branch` + `git gc --prune=now`），消除公开前最后一个 BLOCK 项；目录现仅保留样例复现指引。

### 安全（Security）
- 全仓库密钥 / 凭据（`.env`、Pixiv Cookie、API Key、GitHub Token）不入库；`.workbuddy/` 已忽略。
- 第三方版权样图（`动漫图片测试下载/`）已从工作区与历史清除，目录仅保留复现指引；后续新增媒体须为原创或已授权。
