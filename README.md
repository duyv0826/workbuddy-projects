...
> **公开仓库提示**：本项目已于 2026-08-26 正式转为 **公开（Public）**，对所有用户可见。`动漫图片测试下载/` 目录原先包含的第三方版权样图，已从工作区与 git 历史中彻底清除（目录现仅保留样例复现指引），因此公开前无版权阻塞项。若日后自行添加任何媒体，仍须为原创或已获授权；安全与归属约定见 [SECURITY.md](./SECURITY.md)。
