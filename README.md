...
> **私有仓库提示**：本项目当前为 **私有（Private）** 状态。`动漫图片测试下载/` 目录原先包含的第三方版权样图，已从工作区与 git 历史中彻底清除（目录现仅保留样例复现指引），公开前的版权阻塞项已解除。计划转为公开（Public）前，仍请先阅读 [SECURITY.md](./SECURITY.md) 与 `动漫图片测试下载/README.md`；若日后自行添加任何媒体，须为原创或已获授权。
...
| [`动漫图片测试下载`](./动漫图片测试下载/README.md) | 下载器端到端实测说明与样例复现指引（样图不随仓库分发） | — | — |
...
| 动漫图片测试下载 | [README.md](./动漫图片测试下载/README.md) | 下载器端到端实测说明与样例复现指引（样图不随仓库分发，按需用下载器生成）。 |
...
注意：仓库内 `动漫图片测试下载/` 目录**不再随附任何图片**——原先从公开图站抓取的第三方版权样图，已从工作区与 git 历史中彻底清除，目录现仅保留「如何按需用下载器生成等价 SFW 样图」的复现指引。本仓库 MIT 许可证仅覆盖代码与文档；若日后自行添加媒体，请确保为原创或已获授权（详见该子目录 README）。
